---
lab:
  title: Migrate a MongoDB database to an Azure DocumentDB (with MongoDB compatibility) account
  module: Module 2 - Migrate to Azure DocumentDB (with MongoDB compatibility)
  description: 'In this lab, you learn how to use the MongoDB native tools to migrate a MongoDB database to a Azure DocumentDB (with MongoDB compatibility) account. You use the following tools:'
...
Data migration is a critical step in the process of moving from an existing MongoDB database to a Azure DocumentDB (with MongoDB compatibility) account. While there are several ways to migrate a MongoDB database to a Azure DocumentDB (with MongoDB compatibility) account, this lab focuses on using the MongoDB native tools to migrate the database. The MongoDB native tools are the most common way to migrate a MongoDB database to another. Most MongoDB administrators and developers are familiar with these tools.
...
In this lab, you learn how to use the MongoDB native tools to migrate a MongoDB database to a Azure DocumentDB (with MongoDB compatibility) account. You use the following tools:

- **mongodump**: This tool is used to dump the data from the local MongoDB server into a BSON (Binary JSON) file.
- **mongorestore**: This tool is used to restore the dumped data into the Azure DocumentDB (with MongoDB compatibility) server.
...
If you already have an existing Resource Group, a Azure DocumentDB (with MongoDB compatibility) account, an Azure Storage Account or an Azure Log Analytics workspace that you would like to use, just fill in those values in the **.env** file and set the skip create option for that resource to **true**. By default, the *create-azure-resources.ps1* script uses this file to retrieve the necessary environment variables. Note that the *create-azure-resources.ps1* script will populate the environment variables with default values if not specified in the .env file.
...
We should now have a running MongoDB server with some data into it, it's time to migrate it to Azure DocumentDB (with MongoDB compatibility).

## Migrate to a Azure DocumentDB (with MongoDB compatibility) account using MongoDB native tools (offline)

To export the data from the local MongoDB server and import it into the Azure DocumentDB (with MongoDB compatibility) account, you use the MongoDB **mongodump** and **mongorestore** native tools. In a production environment, if your database is large, you might need to search for other ways to migrate the database like through *Azure Data Migration Service*.
...
1. Restore the dumped data into your Azure DocumentDB (with MongoDB compatibility) server. Replace **<user>**, **<password>**, **yourMongoDBClusterName**, and **yourDatabaseName** with your actual username, password, Azure MongoDB cluster name, and database name:
    ...
    This command restores the BSON file into your Azure DocumentDB (with MongoDB compatibility) server. The **mongorestore** command  outputs the progress of the restore operation.

>[!note]
> Don't forget to make sure the firewall rules for your Azure DocumentDB (with MongoDB compatibility) account are set to allow access from your current public IP address. If your Azure Cosmos DB account was generated by the *create-azure-resources.ps1* script, it should have created the firewall rule for you. Check the existing firewall rules under the ***Networking*** *Setting* section of the *Azure DocumentDB (with MongoDB compatibility)* account. If you are not sure what your current public IP address is, you can use the following command to find out:
> ```powershell
> Invoke-RestMethod -Uri 'http://ipinfo.io/ip' -Method Get
> ```
...
1. Connect to your Azure DocumentDB (with MongoDB compatibility) connection string.
...
>[!note]
> Note that while you used the **mongodump** and **mongorestore** tools to migrate the data from your local MongoDB server to the Azure DocumentDB (with MongoDB compatibility) account, we could have also use **mongoexport** and **mongoimport** tools to do the same.

You should now have a running Azure DocumentDB (with MongoDB compatibility) server with the data from your local MongoDB server. You successfully migrated the data from your local MongoDB server to the Azure DocumentDB (with MongoDB compatibility) account using the MongoDB native tools.
...
You successfully migrated a MongoDB database to an Azure DocumentDB (with MongoDB compatibility) account using the MongoDB native tools. There are several other ways to migrate a MongoDB database to an Azure DocumentDB (with MongoDB compatibility) account, including using Azure Data Studio for offline migrations and Azure Databricks for online/offline migrations. The method you choose depends on your specific requirements and constraints.
